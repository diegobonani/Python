...
def gerenciar_estudos(request):
    ...
    if not ids_selecionados:
        # Se nenhum filtro vier, por padrão mostra todos os ativos (ou lógica que preferir)
        # Aqui assumindo que se vazio via GET inicial, talvez mostrar todos, 
        # mas mantendo a lógica original de filtrar se vier na query.
        # Se vazio no request.GET, pega todos os ids das pessoas ativas para exibir por padrão
        if not request.GET:
             alunos_na_tela = pessoas_ativas
             ids_selecionados = [str(p.id) for p in pessoas_ativas]
        else:
        ...
        for aluno in alunos_na_tela:
            ...
            if exibir_aba_cursos:
                ...
                for disc in disciplinas_raw:
                    # Lógica para Cursos baseados em Catálogo (Estilo Netflix)
                    if disc.curso_catalogo:
                        ...
                        # Se já viu tudo ou não começou, pega a primeira ou fica None
                        if not proxima_aula and total > 0 and concluidas < total:
                             proxima_aula = aulas_curso.first()
                        
                        disc.id_proxima_aula = proxima_aula.id if proxima_aula else None
                    
                    # Lógica para Disciplinas Manuais
                    else:
    ...
    if not usando_local:
        if aula_atual.video_link and "tg://" in aula_atual.video_link:
            ...
            try:
                ...
                # Gera URL do Streamer Local (Ex: TG Streamer)
...
def adicionar_disciplina(request):
    if request.method == 'POST':
        aluno_id = request.POST.get('aluno_id')
        categoria = request.POST.get('categoria', 'EXTRA') # Captura a categoria do Select2 (TI, RH, etc.)
        ...
        if form.is_valid() and aluno_id:
            ...
            disciplina.save()
            messages.success(request, f"Disciplina '{nome_curso}' criada na área {categoria}!")
...
def editar_disciplina(request, disciplina_id):
    ...
    if request.method == 'POST':
        ...
        nova_categoria = request.POST.get('nova_categoria') # Captura a nova categoria
